...
> Tushare 积分: **10000+**  
> 关联文档: [盘后数据自动化-功能说明.md](./盘后数据自动化-功能说明.md) · [盘后数据自动化-施工方案.md](./盘后数据自动化-施工方案.md)（施工时严格按本审计的字段名/单位/口径实现）
...
| 11 | `dc_index.level` 字段全 null（descriptor 列了但实际不填） | 忽略此字段 |
...
| `dc_index` (idx_type=概念板块) | 板块涨跌幅 + 龙头 + 上涨家数 | `pct_change`/`leading`/`up_num` | 单日 486 行 |
...
### 3.3 dc_index — 东财概念板块（必填 `idx_type`）
...
| `idx_type` | str | `"概念板块"` | 板块类型 |
| `level` | null | `null` | ⚠️ **全为 null，忽略** |

**数据规模**: 20260525 当天概念板块 **486 条**（与 20260522 完全一致，说明板块总数稳定）
...
*本审计是 [盘后数据自动化-施工方案.md](./盘后数据自动化-施工方案.md) 的字段层依据。*  
