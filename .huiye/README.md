...
> 最后更新: 2026-05-29 02:15（**命中率与方向算法重设·利空全量剔除·题材按强度排序上线**：A 股做不了空——`strength_score ≤ 0`（利空 + 中性）题材在打分入口直接 raise `SkippedTheme` 跳过，service 层静默吞掉不计入失败统计；4 处聚合 SQL 加 `WHERE strength_score > 0` 防御性过滤。**新命中率公式**：标的级 `is_hit = (pct_chg − zz1000) > strength_score / 20`（α 自适应阈值）→ 标的累计命中率 = 命中天数/有效天数 → 题材命中率 = 标的命中率均值 → 报告命中率 = 题材命中率均值 → 模板命中率 = 报告命中率均值（4 层金字塔）。**新方向公式**：题材每日 INSERT 后二阶段 UPDATE direction_correct，累计判定 = `∏(1 + theme_pct_i / 100) > 1`（D+N 累乘）；NULL theme_pct 行视为因子 1 跳过。**排序**：`get_theme_eval_for_report` SQL `ORDER BY strength_score DESC, priority_rank ASC NULLS LAST, id ASC`。**GUI**：评估页第 2 层看空 / 中性题材消失 + 强度从大到小 + 命中率 hover 显示方向 ✓/✗ + 题材命中率单元格按方向染色（准确浅红 / 不准浅绿）；标的级用 SQL 新字段 `stock_hit_rate` / `valid_days` 显示 "命中/有效 (X%)" + D/α+N 列尾加 ✓/✗ 单日命中标记。**CLI**：新建 `tools/purge_scoring_data.py` 整库清空 theme_prediction_scores + theme_stock_scores（自动备份 + 干跑 + JSON 输出 + 真删需输入 yes 确认）。**验证**：test_script_scorer 21/21（+5 用例 case_17~21 测累乘累计 / α 阈值 / SkippedTheme / NULL 跳过）+ test_scoring_eval 28/28（+4 用例 case_25~28 测利空过滤 + 4 层聚合 + 标的命中率字段）+ GUI 离屏冒烟通过。**主人需手动跑** `python tools\purge_scoring_data.py` 清空旧口径数据，下次调度自动按新口径重写。[上线日志](../doc/updates/05-29-0215-命中率与方向算法重设上线.md) / [内容文档](../doc/design/05-29-0058-命中率与方向算法重设与利空过滤设计.md) / [施工方案](../doc/design/05-29-0110-命中率与方向算法重设施工方案.md)）
> 上一更新: 2026-05-28 23:26（**提示词模板编写规范长期文档归档**：主人新建模板时希望让 AI 一次性写对，要求一份"模板必须遵守的样式"。辉夜核 `core/prompt_loader.py` + `core/ai_news_analyzer.py::build_user_prompt` 渲染逻辑后，把以下规则落成 [doc/guides/05-28-2326-提示词模板编写规范.md](../doc/guides/05-28-2326-提示词模板编写规范.md)：①文件位置 / 命名（`prompts/analysis/{id}.md` + id 必与文件名一致）②强制三段式骨架（`---` frontmatter + `## SYSTEM` + `## USER`）③frontmatter 字段表（id/name/category 必填，version 必须加引号否则被解析成 float）④**占位符规则修订**——只支持 `{max_sectors}` `{stocks_per_sector}` 两个；`{news_data}` / `{market_summary}` **明确不要预留**（写了会被 `format()` 置空，真实数据由系统强制追加到末尾，预留反而留空白割裂），同时讲清楚 Prompt Cache 友好布局策略⑤裸花括号必须 `{{` `}}` 转义 + 自创占位符必抛 KeyError⑥`{{include:NAME}}` 共享片段约束⑦内容硬性约定（锚点必须 `[新闻X](#新闻X)` 单条 + USER 写「以上」不写「以下」因数据追加在末尾）⑧最小骨架模板可直接喂 AI⑨自检流程 `python -m core.prompt_loader`⑩完整调用链 + 11 个 Q&A + 6 份现有样板模板对照表。同时主人下令"记录下长期文档"——按 `.huiye/` vs `doc/` 边界规则，guides 类长期文档不属于 `.huiye/` 4 类白名单，全部归档至 `doc/guides/`。`.huiye/README.md` 加链接已同步。[长期规范](../doc/guides/05-28-2326-提示词模板编写规范.md)）
> 上一更新: 2026-05-28 23:15（**AI 批量分析 12 任务只落 3 份报告 hotfix · 同名互覆**：23:00 批量上线后主人立即反馈"12 个模板全选跑完只看到 3 份报告"。**根因**：`core/ai_news_analyzer.py::save_report` 文件名只到分钟精度 `{月}{日}_{时}{分}_盘后总结分析报告{tail}.md`，`tail` 由 `extra_suffix` 决定；`services/analysis_service.py::AnalysisService.analyze` 仅在**回测路径**（`simulated_trade_date` 给定）填 `_{tpl}_{HHMMSS}` 后缀，**真实生成路径**永远空着 → 12 任务在 23:07 这一分钟内并发完成全部要写同一文件名 → 磁盘后写覆盖先写 + `ai_reports.file_path UNIQUE` 拒绝 11 条 INSERT（IntegrityError 被 service 吞在日志里）→ DB 只剩 3 条且分散在 3 个不同分钟。**修复**：`AnalysisService.analyze` 把 `extra_suffix = f"{sanitized_tpl}_{run_stamp}"` 计算移出 `if simulated_trade_date:` 块，让真实生成 / 回测**两条路径都享受** template_id + HHMMSS 后缀。`save_report` 内部 `elif extra_suffix: tail = f"_{extra_suffix}"` 拼装逻辑无需改动。**结果**：批量分析文件名形如 `5月28日_23时07分_盘后总结分析报告_custom_6_230751.md`，12 模板并发再不会撞。**遗留**：已损坏的 9 份报告永久丢失（磁盘被覆盖 + DB 未入库），主人需重新选这 9 个模板再跑一遍；`save_report` 长远应直接秒级精度 + 把 HHMMSS 责任收回 core 层，列为 TODO 本次不动。[bugfix 文档](../doc/bugfix/05-28-2315-AI批量分析报告同名互覆修复.md)）
> 上一更新: 2026-05-28 23:00（**AI 分析页·批量模板 + 任务队列化上线 + 并发 hotfix 16/64**：主人 22:45 要求把单模板单次的 AI 分析页改造为批量；23:00 hotfix 把并发常量从 3/16 直接拉到 16/64 与手动回测对齐。**改动**：①新建 `gui/workers/ai_analysis_manager.py` ~470 行——`AnalysisTaskManager` + `AnalysisTask` + 5 态机 + relay 信号 + `_emit_finished_once` 去重 helper + 默认 16 并发 / 硬上限 64；②新建 `tools/test_ai_analysis_queue.py` 16 用例（并发限制 / 默认 16 / FIFO / 5 态机 / 删除 / shutdown / clamp 到 64 / 信号去重 case_13）；③`gui/workers/ai_analysis_worker.py` 业务 `finished` → `finished_result`（修复覆盖 `QThread.finished()` 导致调度死锁的隐患） + 所有终态都 emit + 加 `task_id` 字段；④`gui/pages/ai_analysis_page.py` 完整重写：模板 ComboBox→QListWidget 多选 + 左右两栏配置 + 中段 splitter（左=进度 / 右=任务队列）+ 下方结果联动；⑤`range_preset_combo` 默认「上一收盘日 14:00 至今」+ showEvent 主动触发应用；⑥`gui/main_window.py` closeEvent 加 ai_analysis / manual_backtest 两页 manager.shutdown 兜底（Qt 不会递归触发子页 closeEvent）。**关键决策**：①worker 用 `finished_result` 避开 QThread 自带 finished 同名重载坑（手动回测早就遇到了）；②task_finished 去重 emit 保证 page 收到恰好一次；③共用一份盘后总结/时间窗（决策 D3/D4 共用）；④≥3 任务弹确认框防手抖；⑤队列表放中段右半（仿手动回测）。**验证**：test_ai_analysis_queue 16/16 + test_manual_backtest_queue 14/14 + test_theme_extract_queue 14/14 全绿 + MainWindow 完整启动冒烟 OK。[上线日志](../doc/updates/05-28-2300-AI分析页批量模板与队列化上线.md)）
> 上一更新: 2026-05-28 22:30（**评估页「提取题材 / 全部提取 / 批量删除」+ 预测题材页「任务队列」上线**：6 Phase 全跑完，主人 21:30 截图反馈"5 月 26-27 日满屏✗无题材灰按钮"问题彻底解决。**新增**：①`gui/workers/theme_extract_manager.py` ~430 行——`ThemeExtractTaskManager` + `ThemeExtractTask` + 5 态机（PENDING/RUNNING/SUCCESS/SKIPPED/FAILED）+ 默认 16 并发 / 硬上限 64 + relay 信号跨线程安全 + `has_active_for_path` 拦截批量删除冲突；②`tools/test_theme_extract_queue.py` 14 用例（concurrency / FIFO / signals / shutdown / clamp / has_active_for_path / basename）。**改动**：①`gui/workers/theme_extract_worker.py` 4 业务信号首位都加 task_id；②`gui/main_window.py` 持有 manager 单例 + closeEvent shutdown + pages 创建后调 setter 注入两个页面；③`gui/pages/theme_prediction_page.py` 上半区改 `QSplitter(Qt.Horizontal)` 左右两栏（左=原"手动抽取"UI / 右=新"📋 任务队列"4 列+并发 spin+清完成按钮）+ `start_extract` 改入队 + 9 个 manager 信号槽；④`gui/pages/prompt_eval_page.py` 树启用 `ExtendedSelection` 多选 + 行内"✗ 无题材"改可点击"🎯 提取题材" + 表头加"⚡ 全部提取（未抽题材）"+"🗑 批量删除" 2 按钮 + 5 个槽（`_on_extract_one_clicked / _on_batch_extract_all / _on_batch_delete / _on_extract_task_finished / set_theme_extract_manager`）。**关键决策**：新造 manager 不抽公共基类（字段差异大）/ MainWindow 持有跨页单例 / 0 新增 service 接口 / 0 新增 CLI（`delete_report` / `get_report_eval` / `extract_from_file` 全部复用）/ 评估页 toast 不切 Tab / 树原生 Ctrl+Shift 多选不加 checkbox 列 / 含真实报告时强提示二次确认（输入"我确认删除"）。**验证**：test_theme_extract_queue 14/14 + test_manual_backtest_queue 14/14 + test_scoring_eval 24/24 + test_script_scorer 16/16 + GUI 全模块 import 实例化 OK。[上线日志](../doc/updates/05-28-2230-评估页提取题材与队列化上线.md) / [内容设计](../doc/design/05-28-2140-评估页提取题材与队列化设计.md) / [施工方案](../doc/design/05-28-2150-评估页提取题材与队列化施工方案.md)）
> 上一更新: 2026-05-28 22:30（**α 体系金字塔重构上线**：22:00 上 α+N 后主人发现 α / α-1 / α+N 跨层级口径混乱（基准上证综指 / theme_pct−zz1000 / sector−zz1000 三种混着用 + 模板 α+N 一步加权而非两步聚合），直接重构为「报告→模板」严格金字塔。**新口径**：①报告 α+N = SUM(\|str\|·(theme_pct−zz1000)) / SUM(\|str\|)，仅 strength>0；②报告 α = AVG(报告α+1..+5) 忽略 NULL；③报告 α-1 = AVG(报告α+2..+5)；④模板 α+N = AVG over reports of (报告α+N) 简单均值（每份报告等权）；⑤模板 α / α-1 同理从模板 α+N 派生；⑥题材级独立体系：α+N = sector−zz1000 / α = AVG over 5 days / α-1 = AVG over 4 days，与该层界面 D+N 同基础；⑦标的级 α+N = pct_chg−zz1000 不变。**改动**：①迁移 004 `DROP COLUMN theme_prediction_scores.alpha`（原 theme_pct−上证综指 单日落库字段废弃，SQLite 3.49 支持）；②`script_scorer.py` ThemeDailyScore 移除 alpha 字段 + UPSERT 移除 alpha 列 + 计算流程移除 alpha；③`scoring_service.get_report_eval` 重写为嵌套 CTE（per_theme→per_report→外层派生 alpha_avg/alpha_avg_excl_d1）；④`scoring_service.get_template_eval` 重写为 4 层 CTE（per_theme→per_report→tpl_basics+tpl_aggs LEFT JOIN）；⑤`scoring_service.get_theme_eval_for_report` 题材级 α / α-1 改为 AVG over 5/4 days of (sector−zz1000)；⑥`tools/query_eval.py` 单题材逐日列名 alpha→α+N 现推 theme_pct−zz1000；⑦`tools/score_themes.py` _print_score 同步现推；⑧test_scoring_eval / test_script_scorer 夹具 INSERT 全改 + 6 处 alpha 断言重写（case_13 重命名 13_theme_pct_weighted）。**验证**：test_scoring_eval 24/24 + test_script_scorer 16/16 + GUI 模块 import OK + CLI smoke OK。**GUI 零变动**——字段名 a_n_avg / alpha_avg / alpha_avg_excl_d1 没变，新口径直接生效。[上线日志](../doc/updates/05-28-2230-α体系金字塔重构上线.md)）
> 上一更新: 2026-05-28 22:00（**评估页 D+N → α+N / D/α+N 列改造上线**：α+N = D+N − 当天中证1000 涨跌幅，与 α-1 同基准但**逐日单点值**。①**SQL 层** `services/scoring/scoring_service.py` 4 处 SQL 都加 a1~a5 字段：`get_template_eval` per_theme CTE 加 sector−zz1000 透视 + 外层 a_n_avg 加权聚合（仅 strength>0）；`get_report_eval` per_theme CTE 加 theme−zz1000 透视 + 外层 a_n_avg 加权聚合；`get_theme_eval_for_report` 加 sector−zz1000 题材级直透；`get_stock_scores_for_theme` 标的级 LEFT JOIN tps 取 zz1000 + pct_chg−zz1000 透视。②**GUI 上表**：`_TPL_COLS` D+1~D+5 直接改名 **α+1~α+5**（列宽 75 不变），`_render_template_table` 改读 `a_n_avg`；CSV [模板汇总] section 同步改写。③**GUI 下表（树形）**：`_TREE_COLS` D+1~D+5 改名 **D/α+1~D/α+5**（列宽 75→120 容纳双值），新增 `_fmt_pct_dual(d, a) → "+2.50% / +1.20%"`，3 处渲染（报告/题材/标的）的 5 个单元格都改双值；染色 / 排序辅助槽继续按 D 值保留原行为。④**CSV** 4 个 section 都补 a_n 字段：[报告级] 加 5 列 a_n_avg、[题材级] 加 5 列 a_n、[标的级] 加 5 列 a_n_pct。**口径决策**：α+N 与 D+N **同基础**——题材级 α+N 用 sector−zz1000（与该层 D+N 一致），标的级 zz1000 借同 (theme_id, score_date) 的 tps 行 join 取（tss 不冗余存）；原 α / α-1 列**保留**并列展示。**验证**：test_scoring_eval 24/24 PASS（SQL 加列未打挂任何用例）+ GUI 模块 import 冒烟 `_TPL_COLS=16` `_TREE_COLS=14` 索引一致 + `_fmt_pct_dual` 全分支验证 + lint 干净。[上线日志](../doc/updates/05-28-2200-α+N列改造上线.md)）
> 上一更新: 2026-05-28 19:30（**题材抽取 `thinking:disabled` 误发 `deepseek-v4-flash` 致 SSE 死寂 hotfix**：18:20 hotfix 后主人手动回测仍卡死 5+ 分钟（LLM 分析 OK / 题材抽取 200 OK 后流死寂 / 60s read timeout 失灵 / `extract_from_text` 重试压根不触发）。**根因深挖**：`_call_llm` 末尾 `if startswith("deepseek-v4")` 一刀切把 thinking 参数发给 flash 模型——flash **不支持 thinking 参数**，误发后服务端返 200 OK 但 SSE 流死寂（不送 content / 不送 keepalive，TCP keepalive 让 httpx read timeout 失灵）。`_stream_chat` 因主分析跑 pro 不受影响。**改动**：①`core/theme_extractor.py` 一行收紧：`startswith("deepseek-v4")` → `startswith("deepseek-v4-pro")`，flash 不传 thinking；②临时调试 CLI `tools/_debug_theme_extract.py` 逐阶段时间戳 + chunk watchdog（max_gap / chars 实时打印）定位完根因后按规约用后即删；③18:20 hotfix 的 `stream_idle_timeout=60s` + 重试循环保留作真断流/5xx 兜底。**验证**：修复前 5+ 分钟死寂 0 chunk；修复后临时 CLI 跑最新 md 32.8s 跑完、3944 chunks / max_gap=2.2s / finish_reason=stop / 13 题材入库；`test_theme_extract_retry.py` 5/5 + `test_theme_normalize.py` 26/26 全绿；ai_inference.db 当前 40 行题材 / 127 标的 / 96 新闻引用。**教训**：API 参数兼容性按精确模型名判定，前缀通配是这次 bug 的核心。[bugfix 文档](../doc/bugfix/05-28-1930-题材抽取thinking误发flash致SSE死寂.md)）
> 上一更新: 2026-05-28 18:50（**α-1 指标上线 / 中证1000 基准列**：D+1 易因开盘竞价被挂高/挂低导致首日 α 失真，新增 α-1 = D+2~D+5 (theme_pct − 中证1000 pct) 的 4 天均值。**改动**：①迁移 `003_add_benchmark_zz1000.sql` 给 `theme_prediction_scores` 加列 `benchmark_zz1000_pct REAL`（不存衍生 alpha_zz1000，坚持原始数据落库 / 衍生 SQL 推）；②`script_scorer.py` 加 `ZZ1000_TS_CODE='000852.SH'` 常量 + `ThemeDailyScore.benchmark_zz1000_pct` 字段 + UPSERT 加列 + 新增 `backfill_benchmark_zz1000(dry_run)` service 函数；③`scoring_service.py` 三处 SQL（`get_template_eval` / `get_report_eval` / `get_theme_eval_for_report`）都加 `alpha_avg_excl_d1` 衍生字段，口径 `AVG over D+2..D+5 of (theme_pct - benchmark_zz1000_pct)` 题材级，外层与 alpha_avg 同方式聚合；④`gui/pages/prompt_eval_page.py` `_TPL_COLS` 上表加 α-1 列（15→16）+ `_TREE_COLS` 树形加 α-1 列（13→14，`_COL_HIT/_COL_TAIL` 索引顺移）+ 渲染/染色/CSV 三处导出全部覆盖；⑤新增 CLI `tools/backfill_benchmark_zz1000.py --dry-run/--json` 一次性回填老行（当前 scores 表为空，未来同步外部 db 时可用）；⑥`tools/test_script_scorer.py` 新增 case_14（zz1000 写入正确）/ case_15（缺数据 None 兜底）/ case_16（backfill 干跑+真跑），16/16 全绿；`test_scoring_eval.py` 24/24 全绿（SQL 改造未打挂任何用例）。**保留**：原 α（上证综指、5 天均值）不动，并列展示。[上线日志](../doc/updates/05-28-1850-α-1指标上线.md)）
> 上上更新: 2026-05-28 18:20（**题材抽取流式静默自动重试 hotfix**：修复手动回测「题材抽取阶段卡住」长 bug。根因双层：①`theme_extraction.timeout=1200s` 被错误用作 httpx `read` timeout，SSE 流式场景下意味着"chunk 间静默 20 分钟才感知断流"；②`extract_from_text` 完全没有重试机制，单次故障直接放弃；③发现存量未提交半成品代码（`__init__` 改 `self.timeout → stream_idle_timeout / max_attempts` 但 `_call_llm` 还引用已删除字段，AttributeError 被 worker try/except 吞掉表现为"卡住"）。**改动**：①`core/theme_extractor.py` —— `_call_llm` 改用 `self.stream_idle_timeout`；`_http_timeout` 砍到 `read=60s`；`extract_from_text` 包重试循环（最多 3 次含首次 / 退避 2s→4s→8s / 每轮 `progress_callback` 推 UI 提示）；新增 `_is_retryable_error` 判定（超时/SSE 断流/5xx/限流可重试，401/403/400/ValueError 不可重试）；4 级容错全失败也视为"无效响应"重试；部分成功不重试避免重复入库。②`core/ai_config.py` + `config/ai_config.example.json` —— 默认配置补 `stream_idle_timeout=60 / max_attempts=3`；保留 `timeout=1200` 作向后兼容兜底（取 `min(timeout, stream_idle_timeout)`）。③新增 `tools/test_theme_extract_retry.py`（5 用例：首次成功/断流后重试/重试用尽/不可重试立即失败/判定单元，5/5 全绿）+ `tools/test_theme_normalize.py` 4 用例无回归。**主人无感升级**：不动 ai_config.json 直接享受新行为，最坏总耗时 +14s。[bugfix 文档](../doc/bugfix/05-28-1820-题材抽取流式静默自动重试.md)）  
> 上一更新: 2026-05-28 17:55（**手动回测·批量模板 × 批量交易日 + 并发可调（含布局优化 + 上限 64）上线**：把手动回测页从「1 模板 × 1 交易日」改为笛卡尔积批量。①UI 重构（17:55 调整为左右两栏 + 上限提到 64）：模板 `QComboBox` → `QListWidget` 多选（复选框 + 全选/全不选 + "已选 N/M"，左栏占 1/3）；交易日 `QDateEdit` → (开始 ~ 结束) 双 QDateEdit 自动算交易日数（排除今天）；并发硬编码 8 → `QSpinBox 1~64` 默认 8（17:55 hotfix 16 → 64，DeepSeek 60+ req/min 阈值仍能扛）；右栏 2/3 宽叠 3 行：交易日/时间窗/并发+按钮 + 1 行任务数提示；②时间窗策略：单日可微调，多日批量强制每日 (14:00, next_open 09:00) 默认窗；③Manager 加 3 个 API：`MAX_CONCURRENT_HARD_CAP=64` / `get_max_concurrent` / `set_max_concurrent`（运行时调大立即 _pump 拉 PENDING，调小不杀 RUNNING，超出 [1,64] 自动 clamp）；④`_on_run_clicked` 改为笛卡尔积入队，n_tasks ≥ 5 弹确认框（显示模板/日期/时间窗策略/并发/LLM 调用次数）；⑤`_render_md` 移除 8000 字截断，完整渲染；⑥test_manual_backtest_queue 11→14 用例（新加 case_12_grow / case_13_shrink / case_14_clamp 全绿）+ 页面 offscreen 冒烟通过。详见 [上线日志](../doc/updates/05-28-1740-手动回测批量与并发可调.md)）  
> 上一更新: 2026-05-28 17:50（**评估页·一键打分未完成按钮上线**：在「🔁 重打分（按筛选范围）」左侧新增绿色「⚡ 一键打分未完成」按钮——只挑 `score_status ∈ {none, partial}` 的报告逐个调 `rescore_one_report` 增量补齐，已 full 的完全不动，节省市场 API 配额。①service 层新增 `rescore_unfinished_reports` 公开函数 + `BatchUnfinishedResult` dataclass（继承 BatchScoringResult，加 reports_targeted/done/skipped 三字段）；②CLI 入口 `tools/rescore_unfinished.py`（`--days/--time-dim/--is-backtest/--days-back/--json`）；③`gui/workers/rescore_worker.py` 三模式互斥（report_id / unfinished_filter / 区间），新增 `_run_unfinished` 方法；④`gui/pages/prompt_eval_page.py` 控制条 row2 加按钮 + `_on_batch_score_unfinished` 弹窗预扫候选（含初次 N / 续打 M 拆分）→ busy 锁 → worker；⑤docstring 同步更新（按钮区分、调用链）；⑥候选过滤强制 `themes_count > 0`（无题材报告静默跳过，与下表行内按钮禁用态一致）。CLI --help / --json --days 1 / 30 天候选探查（命中 1 个 partial rid=162）/ GUI import 烟测全绿。详见 [上线日志](../doc/updates/05-28-1750-评估页一键打分未完成上线.md)）  
...
| 文档 | 角色 | 状态 |
|------|------|------|
| [⭐ 分析报告查看 & 预测板块体验优化·内容文档](../doc/design/05-29-1010-报告查看与预测板块体验优化设计.md) · [施工方案（第1+2批）](../doc/design/05-29-1025-报告查看与预测板块体验优化施工方案.md) · [上线日志](../doc/updates/05-29-1100-报告查看与预测板块第12批上线.md) | **主人通读版** — 块A 报告查看与联动 + 块B 预测板块筛选查看；4 批次推进建议 + 10 个决策点。**第 1+2 批 6 项全部上线**：Ph1 数据管理页修 `data/analysis→data/AI_analysis` 并支持递归+setMarkdown 渲染 / Ph1 弱匹配阈值题材页 0.7→0.5 与评估页对齐 / Ph2 AI 分析页加「实时流式+成品报告」双 Tab 免开 Typora / Ph3 题材表板块列加中文名 + 新增 `get_sector_names` service+CLI / Ph4 「📊 打分明细」Tab 接通 `get_theme_score_detail` 真数据（顶部摘要 + 逐日表 + AI 评语 + 三种空态）/ Ph5 评估页 signal 扩 3 参 + 题材页单报告 lock 浏览模式（黄底指示条 + 一键解除 + 切 date/prompt 自动解锁 + 锁定期禁删）+ 新增 `ThemeStore.get_by_ai_report_id` service+CLI。第 3、4 批未做（筛选栏/报告中心/跨日总览/预测↔行情联动）等主人验收后再开 | 🟢 **第 1+2 批已上线**（5 commits），第 3+4 批待主人安排 |
| [⭐ 命中率与方向算法重设·剔除利空·题材排序 内容文档](../doc/design/05-29-0058-命中率与方向算法重设与利空过滤设计.md) · [施工方案](../doc/design/05-29-0110-命中率与方向算法重设施工方案.md) · [上线日志](../doc/updates/05-29-0215-命中率与方向算法重设上线.md) | 利空全量剔除（强度≤0）+ 命中率新公式（α 自适应阈值 = 强度/20）+ 方向新公式（∏(1+r) > 1）+ 题材按强度 DESC 排序 + 整库清空 CLI | 🟢 **已上线**（21+28 用例全绿，主人需手动跑 purge_scoring_data） |
...
| [提示词模板编写规范](../doc/guides/05-28-2326-提示词模板编写规范.md) | `prompts/analysis/*.md` 模板硬性约束（骨架 / frontmatter / 占位符规则 / 锚点格式 / Q&A）— 写新模板前必读，也可直接喂给 AI 当 spec |
