...
> 最后更新: 2026-05-27 19:20（**盘后数据「逐表幂等」缓存语义修订上线** 修复主人发现的"按按钮后全部个股Tab共0只"长期 bug：双层早退 + fact_stock_daily 根本不在 fetcher 工作清单。改造点：①fetcher 14 个 step 加表级自检 + 新增 step 15 调 sync_stock_daily ②fact_limit_stock 4 step 共写无 src 字段 → 默认每次重拉 ③service.build() 缓存命中 fall-through 跑 fetcher 补缺 ④schema 加 breadth.stock_daily_rows 纳入完整度评分 ⑤_purge_trade_date 加 fact_stock_daily；test_fetch_idempotent 4/4 + test_daily_sync 7/7 全绿）
...
| [⭐ 手动回测任务队列并行设计·主人通读版](../doc/design/05-27-2101-手动回测任务队列并行设计.md) | **给主人看** — 把手动回测从「一次一条」改为「连点 N 次排队 + 后台 3 并发 + 任务列表联动三大块」；含 5 个产品形态决策表 + 4 道追加决策 + 7 条 Q&A + 行为约束 7 条 / 排除项 5 条 | 🟡 **等主人通读审核**，过审后写施工方案 |
