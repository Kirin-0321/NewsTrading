...
> 最后更新: 2026-05-28 19:30（**题材抽取 `thinking:disabled` 误发 `deepseek-v4-flash` 致 SSE 死寂 hotfix**：18:20 hotfix 后主人手动回测仍卡死 5+ 分钟（LLM 分析 OK / 题材抽取 200 OK 后流死寂 / 60s read timeout 失灵 / `extract_from_text` 重试压根不触发）。**根因深挖**：`_call_llm` 末尾 `if startswith("deepseek-v4")` 一刀切把 thinking 参数发给 flash 模型——flash **不支持 thinking 参数**，误发后服务端返 200 OK 但 SSE 流死寂（不送 content / 不送 keepalive，TCP keepalive 让 httpx read timeout 失灵）。`_stream_chat` 因主分析跑 pro 不受影响。**改动**：①`core/theme_extractor.py` 一行收紧：`startswith("deepseek-v4")` → `startswith("deepseek-v4-pro")`，flash 不传 thinking；②临时调试 CLI `tools/_debug_theme_extract.py` 逐阶段时间戳 + chunk watchdog（max_gap / chars 实时打印）定位完根因后按规约用后即删；③18:20 hotfix 的 `stream_idle_timeout=60s` + 重试循环保留作真断流/5xx 兜底。**验证**：修复前 5+ 分钟死寂 0 chunk；修复后临时 CLI 跑最新 md 32.8s 跑完、3944 chunks / max_gap=2.2s / finish_reason=stop / 13 题材入库；`test_theme_extract_retry.py` 5/5 + `test_theme_normalize.py` 26/26 全绿；ai_inference.db 当前 40 行题材 / 127 标的 / 96 新闻引用。**教训**：API 参数兼容性按精确模型名判定，前缀通配是这次 bug 的核心。[bugfix 文档](../doc/bugfix/05-28-1930-题材抽取thinking误发flash致SSE死寂.md)）
