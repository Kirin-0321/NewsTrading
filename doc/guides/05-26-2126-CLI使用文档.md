...
> 最后更新：2026-05-29 02:15（新增 `purge_scoring_data.py` 配合命中率算法重设上线）
...
```
日常会用 5 个 ─┬─ python tools/market_fetch.py              # 生成今天盘后总结
              ├─ python tools/market_fetch_backfill.py     # 历史交易日批量回填
              ├─ python tools/export_market_summary.py     # 纯导出已缓存的 MD/JSON（不调 API）
...
一次性 3 个 ─┬─ python tools/migrate_prompts_to_files.py    # M0 已跑过，无需再跑
            ├─ python tools/verify_migration_equivalence.py # M0 验收，可重复跑作回归
            └─ python tools/purge_scoring_data.py          # 打分口径变更上线时清旧打分行
...
### 2b. `tools/export_market_summary.py` — 纯导出已缓存的 Markdown / JSON

**做什么**：从 `market_summaries` 表读缓存的 `summary_json`，用**最新 renderer** 重渲染输出。**不调 Tushare、不调 AI、不写 DB**——秒级完成。

**核心用途**：
- 改 `renderer.py` 后立刻验证渲染效果（无需重跑 build）
- 把某日 MD 喂给 AI 分析
- 主人脚本化生成/发邮件/存档

**调用链**：`market_summaries.summary_json → MarketSummaryRenderer.render_compact → stdout/file`

**参数**：

| 参数 | 默认 | 说明 |
|------|------|------|
| `trade_date` | 最近一个有缓存的交易日 | 位置参数 `YYYYMMDD` |
| `--save PATH` | 关 | 保存到文件（与 `--both` 配合时作前缀，自动加 `.md`/`.json`） |
| `--save-dir DIR` | 关 | 批量模式专用，写到目录 |
| `--recent N` | 关 | 批量导出最近 N 天（需配 `--save-dir`） |
| `--json` | 关 | 输出 JSON 而非 Markdown |
| `--both` | 关 | 同时输出 MD + JSON |
| `--use-cached-md` | 关 | 用 DB 缓存的 `summary_md`（不重渲染，方便对比新旧版） |
| `--quiet` / `-q` | 关 | 去掉 CLI 头部，stdout 仅 MD/JSON 内容 |

**示例**：

```powershell
# 最近一日，stdout 输出 MD（⚠️ PowerShell 重定向 > 会乱码，推荐用 --save）
python tools/export_market_summary.py --quiet

# 指定日 + 存文件（推荐用法，UTF-8 正确）
python tools/export_market_summary.py 20260526 --save data/exports/20260526.md

# 输出 JSON
python tools/export_market_summary.py 20260526 --json --save data/exports/20260526.json

# 同时输出 MD + JSON（save 路径自动加后缀）
...
# 对比新旧 renderer：先存旧版，改完代码后存新版，diff
python tools/export_market_summary.py 20260526 --use-cached-md --save old.md
python tools/export_market_summary.py 20260526 --save new.md
```

**退出码**：`0` 成功；`1` 指定日无缓存（提示主人先跑 backfill）；`2` 参数错误。

**⚠️ PowerShell 编码坑**：PowerShell 的 `>` 重定向默认用 UTF-16 LE，会把 Python stdout 的 UTF-8 字节当乱码写入。**用 `--save PATH` 直接写文件**，写入用的是 Python 的 `write_text(encoding='utf-8')`，永远正确。
...
### 10. `tools/purge_scoring_data.py` — 整库清空打分数据（一次性）
...
# 干跑 + JSON 输出（CI 友好）
python tools/purge_scoring_data.py --dry-run --json

# 真删（先备份再清空，需输入 yes 确认）
python tools/purge_scoring_data.py

# CI / 脚本场景跳过确认
python tools/purge_scoring_data.py --yes
```

清空 `theme_prediction_scores` + `theme_stock_scores` 两张表，**不动** `theme_predictions` / `ai_reports` / `theme_stocks`——题材抽取数据完好保留。

**何时跑**：打分算法 / 命中率口径变更上线后（如 2026-05-29 命中率与方向算法重设上线），旧口径行必须清空避免新旧混算。真删前会先把当前 `data/ai_inference.db` 备份到 `data/backups/ai_inference.db.purge_scoring.{时间戳}.bak`，万一误操作可手动复原。

清空后下次调度（每日 17:00）会自动按新口径填回；想立即补打可跑 `python tools/rescore_unfinished.py`。
