"""market.db 连接与迁移管理（Phase M1）。

提供:

* :class:`MarketDB` — 单一入口，封装连接 / 迁移 / 备份 / 完整性校验。
* :func:`get_market_db` — 进程内单例（懒初始化）。

设计要点
--------
1. **迁移目录**: ``services/market/migrations/`` 下 ``NNN_{name}.sql``
   文件按字典序顺序执行，已应用版本号记录在 ``schema_migrations`` 表。
2. **幂等**: 所有迁移 SQL 必须使用 ``CREATE TABLE IF NOT EXISTS`` /
   ``INSERT OR IGNORE``，允许重复跑。
3. **启动备份**: ``ensure_schema()`` 在跑任何迁移之前都会先复制一份
   ``data/backups/market.db.{yyyymmdd_HHMMSS}.bak``，并清理仅保留最近 7 份。
4. **WAL + 外键 + 长 busy_timeout**: 由 :meth:`connect` 上下文统一启用。
5. **跨库 JOIN**: :meth:`attach_to` 把本库 ATTACH 到外部连接，
   常用于 news.db 与 market.db 的联表查询。

使用示例::

    from services.market import MarketDB

    db = MarketDB()
    db.ensure_schema()        # 第一次启动时自动建 14 张表 + 30+ 游资种子

    with db.connect() as conn:
        rows = conn.execute(
            "SELECT * FROM dim_trader_alias WHERE is_famous=1"
        ).fetchall()
...
DEFAULT_DB_PATH = _PROJECT_ROOT / "data" / "market.db"
...
class MigrationError(MarketDBError):
    """迁移执行失败。"""
...
class MarketDB:
    ...
    def attach_to(
        self, foreign_conn: sqlite3.Connection, alias: str = "m"
    ) -> None:
        """把本 market.db ATTACH 到外部连接，用于跨库 JOIN。

        SQLite 通过 ATTACH 加挂从库后，可以直接用 ``alias.table`` 引用。
        外部代码完成后建议调 ``foreign_conn.execute(f"DETACH DATABASE {alias}")``。
        """
        if not self.path.exists():
            raise MarketDBError(f"market.db 不存在: {self.path}")
    ...
    def ensure_schema(self, *, do_backup: bool = True) -> List[int]:
        ...
        Args:
            do_backup: 若 ``market.db`` 已存在且有待应用迁移，则先备份。
        ...
        with self._lock:
            ...
            if existed and do_backup:
                ...
                except Exception as exc:  # 备份失败不要阻塞迁移
            ...
            self._schema_ready = True
            _log.info("market.db 迁移完成：%s", applied)
            return applied
    ...
    def backup_on_startup(self, *, tag: str = "startup") -> Optional[Path]:
        ...
        若 ``market.db`` 不存在或大小为 0，跳过。
        备份文件命名: ``market.db.{tag}.{YYYYMMDD_HHMMSS}.bak``，
        ...
        target = self.backup_dir / f"market.db.{tag}.{ts}.bak"
    ...
    def backup_to(self, target: Path) -> Path:
        ...
        相比文件拷贝更安全（不会锁库、不会拷到半截）。
        """
        target = Path(target)
        target.parent.mkdir(parents=True, exist_ok=True)
        if not self.path.exists():
            raise MarketDBError(f"源 market.db 不存在: {self.path}")
        ...
        try:
            ...
            src.close()
            dst.close()
        _log.info("backup 完成: %s -> %s", self.path.name, target)
        return target

    def _cleanup_old_backups(self) -> None:
        """仅保留最近 BACKUP_RETENTION 份 market.db.*.bak。"""
        if not self.backup_dir.exists():
            return
        files = sorted(
            self.backup_dir.glob("market.db.*.bak"),
...
_singleton: Optional[MarketDB] = None
...
if __name__ == "__main__":  # pragma: no cover
    ...
    db = MarketDB()
    print(f"[market_db] db_path = {db.path}")
    applied = db.ensure_schema()
    print(f"[market_db] applied migrations = {applied}")

    ok, msg = db.verify_integrity()
    print(f"[market_db] integrity_check = {ok} ({msg})")

    stats = db.get_stats()
    print("[market_db] stats:")
